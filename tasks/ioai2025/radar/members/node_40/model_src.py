...
# node_28 found the honest-holdout tau optimum was MONOTONE toward the grid
# floor (tau=0.01 won and smaller kept improving) -> the averaged softmax is
# UNDER-confident on foreground. C2R6 (a) EXTENDS the grid below 0.01 and
# (b) jointly selects a temperature T<1 that SHARPENS the averaged softmax so
# the fg/bg crossover is driven by better-calibrated probabilities rather than
# an ever-smaller tau. Both (T,tau) are chosen on the SAME honest 200-file
# hold-out and applied identically at test inference.
TAU_GRID = [0.5, 0.2, 0.1, 0.05, 0.02, 0.01, 0.005, 0.002, 0.001]
TEMP_GRID = [0.5, 0.7, 1.0]


def _apply_temperature(probs, T):
    """Sharpen a per-cell probability distribution with temperature T<1.
    probs: (...,C) softmax probabilities (rows sum to 1). Returns a re-
    normalized distribution proportional to p**(1/T). T=1.0 is the identity.
    Guards against underflow so degenerate near-one-hot cells stay valid."""
    if T == 1.0:
        return probs
    p = np.clip(probs, 1e-12, 1.0)
    sharp = p ** (1.0 / T)
    denom = sharp.sum(axis=-1, keepdims=True)
    return sharp / np.clip(denom, 1e-12, None)
...
def select_tau(models, holdout_files, mean, std, batch_size=64, log_fn=None):
    """JOINT (T, tau) sweep on a TRAIN-only hold-out slice; return the pair with
    the best weighted-pixel-accuracy. Standardization stats (mean/std) were fit
    WITHOUT these hold-out files. `models` is the ensemble whose flip-TTA softmax
    is averaged per cell FIRST; a temperature T<1 then SHARPENS the (under-
    confident) averaged softmax before the tau rule so the fg/bg crossover is
    driven by better-calibrated probabilities (C2R6). T=1.0 recovers the
    tau-only baseline."""
    if not isinstance(models, (list, tuple)):
        models = [models]
    ...
    results = {"argmax": base}
    best_T, best_tau, best_wpa = 1.0, None, -1.0
    for T in TEMP_GRID:
        P_T = _apply_temperature(P, T)  # sharpen the averaged softmax
        for tau in TAU_GRID:
            pred = _cost_sensitive_pred(P_T, tau)
            s = weighted_pixel_acc(pred, G)
            results[f"T={T},tau={tau}"] = s
            if s > best_wpa:
                best_wpa, best_T, best_tau = s, T, tau
    # If no (T,tau) beats plain argmax, fall back to argmax (tau=None).
    if best_wpa <= base:
        best_T, best_tau = 1.0, None
        best_wpa = base
    if log_fn is not None:
        log_fn(results, best_T, best_tau, best_wpa, base)
    return best_T, best_tau, best_wpa, base, results
...
# ----------------------------------------------------------------------------
@torch.no_grad()
def predict(models, test_files, mean, std, out_dir, tau, T=1.0, batch_size=64):
    """tau=None -> plain argmax; else the temperature-sharpened cost-sensitive
    rule with the (T, tau) pair selected on the honest hold-out. `models` is the
    ensemble whose flip-TTA softmax is averaged per cell FIRST; the identical T
    from select_tau then sharpens that averaged softmax before the tau rule so
    test-time calibration matches the hold-out selection exactly (C2R6). The
    written raw_test/*.npy is the UNSHARPENED 3-model-averaged softmax so the
    Aggregator can re-derive any (T, tau) rule."""
    if not isinstance(models, (list, tuple)):
        models = [models]
    ...
    for x, names in loader:
        ...
        probs_np = probs.cpu().numpy().astype(np.float32)    # (N,C,H,W) raw (unsharpened)
        if tau is None:
            pred_np = probs_hwc.argmax(axis=-1) - 1          # (N,H,W)
        else:
            probs_T = _apply_temperature(probs_hwc, T)       # sharpen with hold-out T
            pred_np = _cost_sensitive_pred(probs_T, tau)     # (N,H,W)
...
def main():
    ...
    # --- JOINTLY tune (temperature T, tau) on the ensemble-AVERAGED softmax
    #     over the TRAIN-only hold-out slice (average in prob space FIRST,
    #     sharpen with T, then apply the tau rule). C2R6. ---
    def _log_tau(results, best_T, best_tau, best_wpa, base):
        print(f"[cal] (3-seed avg) argmax wpa={base:.5f}")
        for T in TEMP_GRID:
            line = f"[cal] T={T}: "
            line += "  ".join(
                f"tau={tau}:{results[f'T={T},tau={tau}']:.5f}" for tau in TAU_GRID)
            print(line)
        print(f"[cal] selected T={best_T} tau={best_tau} (holdout wpa={best_wpa:.5f}, "
              f"argmax={base:.5f}, delta={best_wpa-base:+.5f})")

    best_T, best_tau, best_wpa, base_wpa, tau_results = select_tau(
        models, holdout_files, mean, std, batch_size=64, log_fn=_log_tau)

    rows = predict(models, test_files, mean, std, out_path, tau=best_tau,
                   T=best_T, batch_size=64)
    csv_path = os.path.join(out_path, "predictions.csv")
    write_predictions_csv(rows, csv_path)
    print(f"[out] wrote {csv_path} with {len(rows)} rows (T={best_T}, tau={best_tau})")
