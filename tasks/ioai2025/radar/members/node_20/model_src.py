...
def main():
    ...
    if smoke:
        ...
        # The model is tiny and the A100 is fast (~27 ms/step), so the 4h wall
        # budget is not the binding constraint -- overfitting on 1800 samples is.
        # Size for a generous ~150 epochs (well past convergence for cosine LR),
        # then clamp so we never exceed a comfortable training-wall ceiling.
