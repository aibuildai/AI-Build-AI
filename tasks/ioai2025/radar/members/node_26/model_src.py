...
class Branch(nn.Module):
    """One per-view encoder: 4 double-conv stages [32,64,128,256] with maxpool
    (reaching /8)."""
    def __init__(self, cin=2, widths=(32, 64, 128, 256)):
        super().__init__()
        self.enc1 = DoubleConv(cin, widths[0])
        self.enc2 = DoubleConv(widths[0], widths[1])
        self.enc3 = DoubleConv(widths[1], widths[2])
        self.enc4 = DoubleConv(widths[2], widths[3])
...
class SumFusion(nn.Module):
    ...
    C-channel maps. No concat, no 3C->C projection. A BN+ReLU normalizes the
    summed result to keep the decoder input distribution well-conditioned."""
    def __init__(self, c, reduction=4):
        super().__init__()
...
class MultiBranchViewFusionUNet(nn.Module):
    def __init__(self, widths=(32, 64, 128, 256)):
        ...
        self.fuse4 = SumFusion(widths[3])
        # decoder: from fused f4 (/8) up to f3 (/4), f2 (/2), then f1 (full)
        self.up3 = Up(widths[3], widths[2], widths[2])
    ...
    def forward(self, x):
        ...
        b = x[:, 2:4]
        c = x[:, 4:6]
        a1, a2, a3, a4 = self.branchA(a)
        b1, b2, b3, b4 = self.branchB(b)
        c1, c2, c3, c4 = self.branchC(c)
        f1 = self.fuse1(a1, b1, c1)
        f2 = self.fuse2(a2, b2, c2)
        f3 = self.fuse3(a3, b3, c3)
        f4 = self.fuse4(a4, b4, c4)
        d3 = self.up3(f4, f3)      # /4
        d2 = self.up2(d3, f2)      # /2
...
TAU_GRID = [0.5, 0.2, 0.1, 0.05, 0.02, 0.01]
...
@torch.no_grad()
def _softmax_tta(model, x, use_amp):
    """flip-TTA-averaged softmax, (N,C,H,W) -> numpy (N,C,H,W)."""
    with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=use_amp):
        logits = model(x)
        logits_f = model(torch.flip(x, dims=[3]))
        logits_f = torch.flip(logits_f, dims=[3])
    probs = (F.softmax(logits.float(), dim=1) + F.softmax(logits_f.float(), dim=1)) / 2
    return probs


@torch.no_grad()
def select_tau(model, holdout_files, mean, std, batch_size=64, log_fn=None):
    ...
    for x, names in loader:
        x = x.to(DEVICE, non_blocking=True)
        probs = _softmax_tta(model, x, use_amp)  # (N,C,H,W)
...
# ----------------------------------------------------------------------------
@torch.no_grad()
def predict(model, test_files, mean, std, out_dir, tau, batch_size=64):
...
def main():
    ...
    if smoke:
        ...
        # Deeper/wider model (4-stage [32,64,128,256]); step time is re-measured
        # each run by the micro-benchmark above, so max_steps is re-sized to the
        # measured throughput. Size for a generous ~150 epochs (well past cosine
        # convergence), then clamp to a 40-min training-wall ceiling.
        steps_per_epoch = max(1, len(train_files) // 32)
        max_steps = 150 * steps_per_epoch  # ~8400 steps
        # Never let the training wall exceed ~40 min regardless of step time.
        wall_cap = int(40 * 60 / step_t)
        max_steps = min(max_steps, wall_cap)
        # Ensure a sane floor.
        max_steps = max(max_steps, 40 * steps_per_epoch)
    print(f"[plan] max_steps={max_steps} (est {max_steps*step_t/60:.1f} min)")

    log_path = os.path.join(out_path, "training_progress.jsonl")
    # Train on the FULL train set (preserves node_2's data volume / 0.9358 base).
    model = train(train_files, mean, std, max_steps, batch_size=32, log_path=log_path)
    ...
    best_tau, best_wpa, base_wpa, tau_results = select_tau(
        model, holdout_files, mean, std, batch_size=64, log_fn=_log_tau)

    rows = predict(model, test_files, mean, std, out_path, tau=best_tau, batch_size=64)
    ...
    # save model checkpoint + sentinel
