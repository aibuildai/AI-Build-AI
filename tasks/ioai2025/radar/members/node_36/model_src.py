...
range-velocity), each width [24,48,96], fused at every scale by a learned-sum
fusion (per-branch SE channel gate then element-wise sum), feeding a shared
bilinear-upsample U-Net decoder to a
...
class _ChannelGate(nn.Module):
    """SE-style per-channel sigmoid gate on a single C-channel feature map."""
    ...
    def forward(self, x):
        ...
        s = torch.sigmoid(self.fc2(s)).unsqueeze(-1).unsqueeze(-1)  # (N,C,1,1)
        return x * s


class SumFusion(nn.Module):
    """Learned-sum fusion: gate each branch's C-channel feature independently
    with its own SE-style channel gate, then element-wise SUM the three gated
    C-channel maps (three per-view branches). No concat, no projection. A
    BN+ReLU normalizes the summed result to keep the decoder input distribution
    well-conditioned. (node_20 3-branch design -- the lineage best at 0.9552.)"""
    def __init__(self, c, reduction=4):
        super().__init__()
        self.gate_a = _ChannelGate(c, reduction)
        self.gate_b = _ChannelGate(c, reduction)
        self.gate_c = _ChannelGate(c, reduction)
        self.bn = nn.BatchNorm2d(c)

    def forward(self, a, b, c):
        y = (self.gate_a(a) + self.gate_b(b) + self.gate_c(c))  # (N,C,h,w)
...
class MultiBranchViewFusionUNet(nn.Module):
    def __init__(self, widths=(24, 48, 96)):
        ...
        self.fuse1 = SumFusion(widths[0])
        self.fuse2 = SumFusion(widths[1])
        self.fuse3 = SumFusion(widths[2])
...
def train(train_files, mean, std, max_steps, batch_size=32, log_path=None, seed=1234):
    # Re-seed per-run so each seed produces an independent-weight model but sees
    # the same standardized data (decorrelation comes from init + shuffle/aug RNG).
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)
    ...
    def log_event(event_type, extra=None):
        ...
        rec = {"step": step, "elapsed_seconds": round(time.time() - t0, 2),
               "event_type": event_type, "metrics": extra or {}}
...
TAU_GRID = [0.5, 0.3, 0.2, 0.15, 0.1, 0.05, 0.02]
...
def _softmax_tta(models, x, use_amp):
    """Multi-seed + flip-TTA averaged softmax in PROBABILITY space.
    `models` is a single model or a list; each contributes its flip-TTA
    softmax and they are averaged equally -> (N,C,H,W)."""
    ...
    for model in models:
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=use_amp):
            logits = model(x)
            logits_f = model(torch.flip(x, dims=[3]))
...
def select_tau(models, holdout_files, mean, std, batch_size=64, log_fn=None):
    ...
    `models` may be a list -> their flip-TTA softmaxes are averaged in
    probability space before the cost-rule.
...
def predict(models, test_files, mean, std, out_dir, tau, batch_size=64):
    """tau=None -> plain argmax; else cost-sensitive rule.
    `models` may be a list -> softmaxes averaged in probability space; the
    saved raw_test/*.npy is the multi-seed-averaged softmax."""
...
def main():
    ...
    if smoke:
        ...
        # The model is tiny and the A100 is fast (~30 ms/step), so the 4h wall
        # budget is not the binding constraint -- overfitting on 1800 samples is.
        # Size each seed for a generous ~150 epochs (well past cosine-LR
        # convergence), then clamp the TOTAL training wall (n_seeds runs) to a
        # comfortable ceiling so 3 sequential seeds still fit the budget.
        steps_per_epoch = max(1, len(train_files) // 32)
        max_steps = 150 * steps_per_epoch  # ~8400 steps/run
        # Never let the COMBINED (n_seeds) training wall exceed ~45 min.
        wall_cap_total = int(45 * 60 / (step_t * n_seeds))
        max_steps = min(max_steps, wall_cap_total)
        # Ensure a sane floor.
        max_steps = max(max_steps, 40 * steps_per_epoch)
    print(f"[plan] max_steps={max_steps}/run x{n_seeds} seeds "
          f"(est {max_steps*step_t*n_seeds/60:.1f} min total)")

    # --- Multi-seed ensemble: train 3 independent-seed copies of the SAME
    #     node_20 3-branch architecture on the FULL train set. Their flip-TTA
    #     softmaxes are averaged in probability space (soft-voting) before the
    #     cost-rule for BOTH holdout tau-selection and test prediction. ---
    seeds = [1234] if smoke else [1234, 2025, 777]
    models = []
    for si, sd in enumerate(seeds):
        # each seed appends to a per-seed progress log so traces stay separable
        log_path = os.path.join(out_path, f"training_progress_seed{sd}.jsonl")
        print(f"[train] === seed {sd} ({si+1}/{len(seeds)}) ===")
        m = train(train_files, mean, std, max_steps, batch_size=32,
                  log_path=log_path, seed=sd)
        models.append(m)
    # canonical training_progress.jsonl = the first seed's trace (harness reads it)
    _first_log = os.path.join(out_path, f"training_progress_seed{seeds[0]}.jsonl")
    if os.path.exists(_first_log):
        import shutil
        shutil.copyfile(_first_log, os.path.join(out_path, "training_progress.jsonl"))

    # --- select the cost-sensitive threshold tau on the TRAIN-only hold-out slice ---
    def _log_tau(results, best_tau, best_wpa, base):
        print(f"[tau] argmax wpa={base:.5f}")
        for tau in TAU_GRID:
    ...
    # save all seed EMA checkpoints + sentinel. best_model.pth = seed[0]'s
    # weights (single-model fallback); best_model_seed{sd}.pth = each member.
    torch.save(models[0].state_dict(), os.path.join(out_path, "best_model.pth"))
    for sd, m in zip(seeds, models):
        torch.save(m.state_dict(),
                   os.path.join(out_path, f"best_model_seed{sd}.pth"))
    with open(os.path.join(OUTPUT_DIR, "best_model.pkl"), "wb") as f:
        pickle.dump({"arch": "MultiBranchViewFusionUNet",
                     "seeds": seeds, "n_members": len(models),
