...
# ----------------------------------------------------------------------------- train
def train_model(train_ids, img_dir, den_dir, max_steps, device, log_fn, smoke=False):
    set_seed(SEED)
...
# ----------------------------------------------------------------------------- inference
@torch.no_grad()
def predict(model, img_dir, ids, device):
    """Horizontal-flip test-time averaging.

    For each image, run the model on the original AND on its horizontal mirror,
    un-flip the mirrored density map, and average the two maps. hflip is a verified
    safe invariance here (the count is flip-invariant and hflip is already a training
    augmentation), so averaging over an image and its mirror reduces per-image count
    variance without changing the expected count. calibrate / holdout / test all call
    this same predict(), so the global scale k is fit on the SAME TTA'd distribution
    used for the final test predictions -- keeping calibration consistent.
    """
    model.eval()
    preds = []
    for i in ids:
        ...
        # 2-view TTA: identity + horizontal flip (flip along the width axis, dim=-1).
        t_flip = torch.flip(t, dims=[-1])
        with torch.cuda.amp.autocast():
            out0 = model(t)         # (1,1,180,320)
            out1 = model(t_flip)    # prediction on the mirrored image
        out0 = out0.float().clamp_min(0.0)
        out1 = out1.float().clamp_min(0.0)
        # un-flip the mirrored output so both maps are in the same spatial frame,
        # then average the two density maps.
        out1 = torch.flip(out1, dims=[-1])
        out = 0.5 * (out0 + out1)
        # ensure exact (180,320)
        if out.shape[-2:] != (DEN_H, DEN_W):
            out = F.interpolate(out, size=(DEN_H, DEN_W), mode="bilinear", align_corners=False)
            out = out.clamp_min(0.0)
        preds.append(out.squeeze().cpu().numpy().astype(np.float32))
...
def main():
    ...
    ema_model, raw_model, steady = train_model(
        tr_ids, train_img, train_den, max_steps, device, log_fn, smoke=SMOKE)

    diag = None
    if val_ids:
        gt_counts = np.array([np.load(os.path.join(train_den, f"{i}.npy")).sum() for i in val_ids])
        vpred = predict(ema_model, train_img, val_ids, device)
    ...
    cal_pred = predict(ema_model, train_img, cal_ids, device)
    ...
    if val_ids:
        gt_counts_v = np.array([np.load(os.path.join(train_den, f"{i}.npy")).sum()
                                for i in val_ids])
        vpred_c = predict(ema_model, train_img, val_ids, device) * cal_k
        diag = counting_score(vpred_c, gt_counts_v)
        print(f"[HOLDOUT] counting_score(EMA, calibrated) = {diag:.4f}", flush=True)

    # Test predictions with EMA model, calibrated
    test_preds = predict(ema_model, test_img, test_ids, device)
    ...
    torch.save(ema_model.state_dict(), os.path.join(OUTPUT_DIR, "best_model.pth"))
    with open(os.path.join(OUTPUT_DIR, "best_model.pkl"), "wb") as f:
        import pickle
        pickle.dump({"info": "EMA CSRNet-VGG16BN DM-Count", "steady_step_time": steady,
