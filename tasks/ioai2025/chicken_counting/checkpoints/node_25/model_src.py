"""
Node 25 -- C3R4: Snapshot ensemble on the direct global count regression head.

Same scalar-count architecture as node_24 (ResNet34 + GAP + MLP head + Softplus),
but the single cosine descent is replaced by a snapshot ensemble
(CosineAnnealingWarmRestarts, Huang et al. 2017): M=4 cycles over the SAME total
MAX_STEPS; at the end of each cycle we snapshot the current EMA weights. At inference
we run all M snapshots with hflip TTA and AVERAGE their per-image counts in COUNT
SPACE (before the train-fit alpha calibration). This reduces the per-image count
variance the parent diagnosed as the residual error source, while a single global
alpha fit on the ensemble-mean train counts preserves the mean-bias correction.
...
# --- snapshot-ensemble config (Huang et al. 2017, CosineAnnealingWarmRestarts) ---
# M=4 equal cycles over the SAME total MAX_STEPS; snapshot EMA weights at the end of
# each cycle. EMA is CARRIED across restarts (not reset) -- the coder found this stable
# and it keeps the cheap variance-reduction benefit of averaging M near-minima models.
SNAP_M = int(os.environ.get("SNAP_M", "4"))
...
def main():
    ...
    # --- snapshot ensemble schedule: M equal cosine cycles with warm restarts ---
    # In SMOKE mode we shrink M so at least 1 cycle completes within the tiny step budget.
    snap_m = max(1, min(SNAP_M, max_steps))
    T_cycle = max(1, max_steps // snap_m)
    # cycle-end steps (1-indexed optimizer step at which each cosine cycle reaches LR_MIN);
    # the last snapshot is forced to the final step so the last cycle always contributes.
    snap_steps = [T_cycle * (k + 1) for k in range(snap_m)]
    snap_steps[-1] = max_steps
    snap_steps = sorted(set(min(s, max_steps) for s in snap_steps))
    log(f"snapshot ensemble: M={snap_m} cycles, T_cycle={T_cycle}, snap_steps={snap_steps}")

    # CosineAnnealingWarmRestarts anneals each param-group LR from its initial value down
    # to LR_MIN over T_cycle steps, then restarts. T_mult=1 => equal-length cycles.
    sched = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        opt, T_0=T_cycle, T_mult=1, eta_min=LR_MIN)
    scaler = torch.amp.GradScaler("cuda", enabled=(device.type == "cuda"))

    snapshots = []          # list of EMA state_dicts harvested at cycle ends
    ...
    while step < max_steps:
        ...
        # --- snapshot ensemble: harvest EMA weights at each cosine-cycle minimum ---
        # step==T_cycle*k is the iteration whose opt.step() used the annealed LR minimum
        # (the min LR was set by the PREVIOUS sched.step()); after this iteration's
        # sched.step() the LR restarts to its max. So the weights captured here reflect a
        # well-annealed optimum, even though get_last_lr() already shows the post-restart LR.
        if step in snap_set:
            snap = copy.deepcopy(ema.shadow.state_dict())
            snapshots.append({k: v.detach().cpu().clone() for k, v in snap.items()})
            log(f"snapshot {len(snapshots)} harvested at step {step} (cosine-cycle minimum)")
            emit(step, "snapshot", {"snapshot_idx": len(snapshots)})
    ...
    # --- ensure we always have at least one snapshot (e.g. time-budget cut mid-cycle) ---
    if not snapshots:
        snap = copy.deepcopy(ema.shadow.state_dict())
        snapshots.append({k: v.detach().cpu().clone() for k, v in snap.items()})
        log("no cycle-end snapshot harvested; using final EMA weights as the single snapshot")
    log(f"snapshot ensemble: {len(snapshots)} member(s) for inference")

    # --- snapshot-ensemble inference ---
    # For each image we average the per-image counts (hflip TTA) over the MEMBER snapshots
    # in COUNT SPACE, BEFORE the train-fit alpha calibration. Which snapshots are MEMBERS is
    # decided by a TRAIN-ONLY convergence gate below: with M=4 equal 1000-step cosine cycles,
    # the early cycles do not fully converge (the near-frozen backbone + head need more than
    # one short cycle), so their snapshots collapse to a low count scale (train-count mean far
    # below the converged snapshots). Equal-weight averaging of an undertrained member is NOT
    # variance reduction -- it injects a large scale bias that clamps alpha at its bound. The
    # gate keeps only snapshots whose TRAIN count-scale is consistent with the best snapshot,
    # so the surviving ensemble is the set of CONVERGED cosine-minima -- exactly the diverse
    # optima the snapshot-ensemble method is meant to average. Gate uses TRAIN labels only.
    net = ema.shadow.to(device).eval()  # reused as the loadable module for each snapshot

    def per_snapshot_counts(paths, members):
        """Return array (len(members), N) of per-image counts (hflip TTA) for each member."""
        ds = TestDataset(paths)
        loader = DataLoader(ds, batch_size=32, shuffle=False,
                            num_workers=8 if not SMOKE else 2, pin_memory=True)
        rows = []
        for mi in members:
            net.load_state_dict(snapshots[mi])
            net.eval()
            out = []
            with torch.no_grad():
                for imgs in loader:
                    ...
                    out.append((0.5 * (c0 + c1)).float().cpu().numpy())
            rows.append(np.concatenate(out))
        return np.asarray(rows, dtype=np.float64)  # (n_members, N)

    train_true = np.array([float(np.load(dp).astype(np.float32).sum()) for dp in dens_paths],
                          dtype=np.float64)

    # --- TRAIN-only convergence gate: score each snapshot alone, keep the converged ones ---
    all_members = list(range(len(snapshots)))
    per_snap_train = per_snapshot_counts(img_paths, all_members)  # (M, n_train)
    snap_train_mean = per_snap_train.mean(axis=1)                 # per-snapshot mean count
    best_scale = float(snap_train_mean.max()) if snap_train_mean.size else 0.0
    # keep snapshots within 15% of the best count-scale (drops collapsed/undertrained cycles)
    GATE = 0.85
    keep = [m for m in all_members if snap_train_mean[m] >= GATE * best_scale]
    if not keep:
        keep = [int(np.argmax(snap_train_mean))]
    log(f"snapshot train-count means={np.round(snap_train_mean,2).tolist()} "
        f"best_scale={best_scale:.2f} -> kept members {keep} (gate {GATE})")

    # ensemble-mean TRAIN counts over the kept members (count-space average)
    train_pred = per_snap_train[keep].mean(axis=0)

    # --- train-only global calibration: fit alpha minimizing the exact metric on TRAIN ---
    # bound widened to [0.80,1.40] so the calibration is not clamped if the converged-ensemble
    # mean sits slightly below the parent single-model scale (search still robust, train-only).
    alphas = np.linspace(0.80, 1.40, 601)
    ...
    log(f"train-fit calibration alpha={alpha:.4f} (train err {min(errs):.4f}) "
        f"over {len(keep)} kept snapshot(s)")

    # ensemble-mean TEST counts over the same kept members, then apply the train-fit alpha
    counts_pred = per_snapshot_counts(test_paths, kept_members).mean(axis=0) * alpha  # (N_test,)
    ...
    # checkpoint: store ALL snapshots + prior + fitted alpha so it is self-contained.
    # "model" holds snapshot 1 for backward-compatible single-model loaders.
    torch.save({
        "model": snapshots[kept_members[-1]],  # a converged snapshot for single-model loaders
        "snapshots": snapshots,
        "kept_members": kept_members,
        "prior": prior,
        "alpha": alpha,
    }, os.path.join(OUTPUT_DIR, "best_model.pth"))
    # also emit individual snapshot files snapshot_1..snapshot_M.pth (recipe change 1)
    for i, sd in enumerate(snapshots, start=1):
        torch.save({"model": sd, "prior": prior},
                   os.path.join(OUTPUT_DIR, f"snapshot_{i}.pth"))
